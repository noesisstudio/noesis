...
class BackendTestCase(unittest.TestCase):
    ...
    def test_public_privacy_pages_match_loaded_third_parties(self):
        ...
        self.assertIn("cal.com/bynoesis/agenda-una-llamada-con-nosotros", contact.text)
        self.assertIn("data-cal-consent", contact.text)
        self.assertIn("frame-src https://cal.com", contact.headers["content-security-policy"])
        self.assertIn("frame-src 'none'", privacy.headers["content-security-policy"])
    ...
    def test_public_and_account_pricing_share_the_current_catalog(self):
        ...
        with patch.object(server, "start_scheduler", lambda: None):
            with TestClient(server.app) as client:
                ...
                self.assertIn("conversation-hero", home_page.text)
                self.assertIn("data-conversation-demo", home_page.text)
